...
public class Contacts.PhoneNrsField : PropertyField {
  ...
  protected ArrayList<TypeDescriptor> types = new ArrayList<TypeDescriptor> ();
  protected ArrayList<string> phone_nrs = new ArrayList<string> ();
  protected ArrayList<unowned MultiMap<string, string>?> parameters
      = new ArrayList<unowned MultiMap<string, string>?> ();
  ...
  public PhoneNrsField (Persona persona) {
    ...
    foreach (var phone in details.phone_numbers) {
      var type_descr = TypeSet.phone.lookup_descriptor_for_field_details (phone);
      this.types.add (type_descr);
      this.phone_nrs.add (phone.value);
      this.parameters.add (phone.parameters);
    }
  }
  ...
  protected override void create_widgets (PropertyWidget widget) {
    for (int i = 0; i < this.phone_nrs.size; i++)
      add_field (this.types[i], this.phone_nrs[i], widget);
  }

  protected virtual void add_field (TypeDescriptor type, string phone, PropertyWidget widget) {
    var type_label = row.create_type_label (type.display_name);
...
public class Contacts.EditablePhoneNrsField : PhoneNrsField, EditableProperty {
  ...
  public EditablePhoneNrsField.empty () {
    ...
    this.types.add (TypeSet.phone.lookup_descriptor_in_store ("Mobile"));
    this.phone_nrs.add ("");
    this.parameters.add (null);
  }

  protected override void add_field (TypeDescriptor type, string phone, PropertyWidget widget) {
    var type_combo = row.create_type_combo (TypeSet.phone, type);
    type_combo.changed.connect ((combo) => {
        var row_nr = row.get_child_row (combo);
        this.types[row_nr] = type_combo.active_descriptor;
      });
  ...
  private HashSet<PhoneFieldDetails>? create_set () {
    ...
    for (int i = 0; i < this.phone_nrs.size; i++) {
      ...
      var parameters = this.types[i].add_type_to_parameters (this.parameters[i]);
      var phone = new PhoneFieldDetails (this.phone_nrs[i], parameters);
...
public class Contacts.EmailsField : PropertyField {
  ...
  public EmailsField (Persona persona) {
    ...
    foreach (var email in details.email_addresses) {
      var type_descr = TypeSet.email.lookup_descriptor_for_field_details (email);
      this.types.add (type_descr);
      this.emails.add (email.value);
      this.parameters.add (email.parameters);
    }
  }
  ...
  protected virtual void add_field (TypeDescriptor type, string email, PropertyWidget widget) {
    var type_label = row.create_type_label (type.display_name);
...
public class Contacts.EditableEmailsField : EmailsField, EditableProperty {
  ...
  public EditableEmailsField.empty () {
    ...
    this.types.add (TypeSet.email.lookup_descriptor_in_store ("Personal"));
    this.emails.add ("");
    this.parameters.add (null);
  }

  protected override void add_field (TypeDescriptor type, string email, PropertyWidget widget) {
    var type_combo = row.create_type_combo (TypeSet.email, type);
    type_combo.changed.connect ((combo) => {
        var row_nr = row.get_child_row (combo);
        this.types[row_nr] = type_combo.active_descriptor;
      });
  ...
  private HashSet<EmailFieldDetails>? create_set () {
    ...
    for (int i = 0; i < this.emails.size; i++) {
      ...
      var parameters = this.types[i].add_type_to_parameters (this.parameters[i]);
      var email = new EmailFieldDetails (this.emails[i], parameters);
...
public class Contacts.UrlsField : PropertyField {
  ...
  public UrlsField (Persona persona) {
    ...
    foreach (var url in details.urls) {
      ...
      this.parameters.add (url.parameters);
    }
  }
...
public class Contacts.EditableUrlsField : UrlsField, EditableProperty {
  ...
  private HashSet<UrlFieldDetails>? create_set () {
    ...
    for (int i = 0; i < this.urls.size; i++) {
      ...
      var url = new UrlFieldDetails (this.urls[i], this.parameters[i]);
      new_details.add (url);
    }
...
public class Contacts.NotesField : PropertyField {
  ...
  public NotesField (Persona persona) {
    ...
    foreach (var note in details.notes) {
      ...
      this.parameters.add (note.parameters);
    }
  }
...
public class Contacts.EditableNotesField : NotesField, EditableProperty {
  ...
  private HashSet<NoteFieldDetails>? create_set () {
    ...
    for (int i = 0; i < this.notes.size; i++) {
      ...
      var note = new NoteFieldDetails (this.notes[i], this.parameters[i]);
...
public class Contacts.PostalAddressesField : PropertyField {
  ...
  public PostalAddressesField (Persona persona) {
    ...
    foreach (var address in details.postal_addresses) {
      var type_descr = TypeSet.general.lookup_descriptor_for_field_details (address);
      this.types.add (type_descr);
      this.addresses.add (address.value);
      this.parameters.add (address.parameters);
    }
  }
  ...
  protected override void create_widgets (PropertyWidget widget) {
    for (int i = 0; i < addresses.size; i++)
      add_field (this.types[i], this.addresses[i], widget);
  }

  protected virtual void add_field (TypeDescriptor type, PostalAddress address, PropertyWidget widget) {
    var type_label = row.create_type_label (type.display_name);
    var value_label = row.create_value_label (format_address (address));
    widget.add_row (type_label, value_label);
  }
...
public class Contacts.EditablePostalAddressesField : PostalAddressesField, EditableProperty {
  ...
  public EditablePostalAddressesField.empty () {
    ...
    this.types.add (TypeSet.general.lookup_descriptor_in_store ("Home"));
    this.addresses.add (new PostalAddress ("", "", "", "", "", "", "", "", ""));
    this.parameters.add (null);
  }

  protected override void add_field (TypeDescriptor type, PostalAddress address, PropertyWidget widget) {
    var type_combo = row.create_type_combo (TypeSet.general, type);
    type_combo.changed.connect ((combo) => {
        var row_nr = row.get_child_row (combo);
        this.types[row_nr] = type_combo.active_descriptor;
      });
  ...
  private HashSet<PostalAddressFieldDetails>? create_set () {
    ...
    for (int i = 0; i < this.addresses.size; i++) {
      ...
      var parameters = this.types[i].add_type_to_parameters (this.parameters[i]);
      var address = new PostalAddressFieldDetails (this.addresses[i], parameters);

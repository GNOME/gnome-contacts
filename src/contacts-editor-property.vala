...
public class Contacts.BirthdayEditor : Hdy.Dialog {
  ...
  public BirthdayEditor (Window window, DateTime birthday) {
    ...
    this.add_buttons (_("Set"), ResponseType.OK,
        ...
        if (details != null) {
          ...
          if (!only_new)
            foreach (var email in emails) {
              this.add (create_for_email (details.email_addresses, email));
            }
          if (this.writeable)
            this.add (create_for_email (details.email_addresses));
        }
        break;
      case "phone-numbers":
        ...
        if (details != null) {
          ...
          if (!only_new)
            foreach (var phone in phones) {
              this.add (create_for_phone (details.phone_numbers, phone));
            }
          if (this.writeable)
            this.add (create_for_phone (details.phone_numbers));
        }
        break;
      case "urls":
        var details = p as UrlDetails;
        if (details != null) {
          var urls = ContactUtils.sort_fields<UrlFieldDetails>(details.urls);
          if (!only_new)
            foreach (var url in urls) {
              this.add (create_for_url (details.urls, url));
            }
          this.add (create_for_url (details.urls));
        }
        break;
      case "nickname":
        var name_details = p as NameDetails;
        if (name_details != null && name_details.nickname != null && !only_new) {
          this.add (create_for_nick (name_details));
        }
        break;
      case "birthday":
        var birthday_details = p as BirthdayDetails;
        if (birthday_details != null && !only_new) {
          this.add (create_for_birthday (birthday_details));
        }
        break;
      case "notes":
        ...
        if (note_details != null) {
          if (!only_new)
            foreach (var note in note_details.notes) {
              this.add (create_for_note (note_details.notes, note));
            }
          if (this.writeable)
            this.add (create_for_note (note_details.notes));
        }
        break;
        ...
        if (address_details != null) {
          if (!only_new)
            foreach (var addr in address_details.postal_addresses) {
              this.add (create_for_address (address_details.postal_addresses, addr));
            }
          if (this.writeable)
            this.add (create_for_address (address_details.postal_addresses));
        }
        break;
    }
  }
